...
const manifest = {
  ...
  mandatory: false,
  releaseNotes: "تحديث v1.0.5: تثبيت دائم لورقة المعاينة A4 في إعدادات الطباعة، حظر طبي صارم لطباعة النتائج غير المكتملة مع نوافذ تنبيه سريرية، تحسينات الواجهات والأداء واستقرار إدارة الصندوق والورديات."
};
    ...
    try {
      ...
      if (!release || releaseRes.status === 404) {
        ...
        const createRes = await apiRequest(`/repos/${OWNER}/${REPO}/releases`, 'POST', JSON.stringify({
          ...
          name: `Labryo LIMS ${TAG} - تثبيت ورقة المعاينة، حظر طباعة النتائج غير المكتملة وتحسينات شاملة`,
          body: `## تحديث نظام المختبرات الطبية (${TAG})

### أبرز التحديثات والميزات الجديدة في هذا الإصدار:
1. **تثبيت ورقة المعاينة A4 وعزل حركة الإعدادات**: تثبيت ورقة التقرير A4 بشكل دائم ومطلق أثناء تخصيص وتعديل إعدادات الطباعة، مع شريط تمرير مستقل وسلس للإعدادات.
2. **الرقابة الطبية الصارمة وحظر طباعة النتائج غير المكتملة**: منع إصدار أو طباعة أي تقرير طبي أو مشاركته عبر واتساب لعينة بها تحاليل لم يتم إدخال نتائجها بعد لمنع أي لبس أو خطأ طبي، مع ظهور شارات حالة ملونة ونافذة تنبيه سريرية توضح الفحوصات المتبقية وخيار الحفظ كمسودة.
3. **تنظيف وتنسيق واجهات النظام**: إزالة أي أزرار أو أشرطة سفلية غير مرغوبة وتنظيم شاشة إدخال النتائج لتوفير مساحة عمل سريرية فسيحة.
4. **استقرار المحرك المكتبي وإدارة الورديات**: تحسين كفاءة تتبع الصندوق والورديات اليومية وضمان إغلاق وفتح سلس للتطبيق.
